...
public interface MemberToken<T, S extends MemberToken<T, S>> extends DeclarationToken<S> {
  ...
  /**
   * Derive a new {@link MemberToken} instance, with the given bounds incorporated
   * into the bounds of the underlying resolver. The original {@link MemberToken}
   ...
   * 
   * <p>
   * The new {@link MemberToken} will always have a owner type which is as or more
   * specific than both the current receiver type <em>and</em> the given type.
   ...
   * are less specific that those implied by the <em>current</em> receiver type,
   ...
   * 
   * <p>
   * This may result in unsafe transformations when we convert from a raw receiver
   * to a parameterized receiver, but declarations of those types should give a
   * raw type warning from the Java compiler and this is considered sufficient.
   * 
   * @param type
   ...
   *         intersection type of the current owner type and the given type. That
   *         is, any type which can be assigned to both the given type and the
   *         current owner type, will also be assignable to the new type.
   */
  ...
  default boolean isRaw() {
    return getOwningDeclaration().map(DeclarationToken::isRaw).orElse(false);
  }
}
